...
void serve_file(int client, const char *filename)
{
    ...
    if ((resource = fopen(filename, "r")) == NULL) not_found(client);
    else {
...
int main()
{
    ...
    printf("TinyHTTPd running on localhost by port %d\n", port);
    ...
    while (1) {
        ...
        if (pthread_create(&newthread , NULL, accept_request, (void *)&client_sock) != 0) perror("pthread_create");
    }
